...
describe("Library tests", () => {
  ...
  it("buildSongItem should add a call to HTTPManager.updateSong on click event and change the classList", () => {
    ...
    const songItemLiked = library.buildSongItem({ liked: true }).lastChild;
    const songItemUnliked = library.buildSongItem({ liked: false }).lastChild;

    expect(songItemLiked.classList.toString()).toEqual(songLikedClassList);
    expect(songItemUnliked.classList.toString()).toEqual(songNotLikedClassList);
    songItemLiked.click();
    songItemUnliked.click();
    expect(spy).toHaveBeenCalledTimes(2);
    expect(songItemLiked.classList.toString()).toEqual(songNotLikedClassList);
    expect(songItemUnliked.classList.toString()).toEqual(songLikedClassList);
  });
