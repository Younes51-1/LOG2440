...
describe.only("Player tests", () => {
  ...
  it("loadSongs should load songs in playlist", () => {
    ...
    const songSrc = songStub.src.replace("./", "http://localhost/");
    expect(player.audio.src).toEqual(songSrc);
  });
  ...
  it("playAudio should resume correctly given an index of -1", () => {
    const playerAudioPlaySpy = jest
      .spyOn(player.audio, "play")
      .mockImplementation(() => {});
    const playerAudioPauseSpy = jest
      .spyOn(player.audio, "pause")
      .mockImplementation(() => {});
    const playerAudioLoadSpy = jest
      .spyOn(player.audio, "load")
      .mockImplementation(() => {});
    const playerGetSongFromIndexSpy = jest
      .spyOn(player, "getSongFromIndex")
      .mockImplementation(() => {});
  ...
  it("playPreviousSong & playNextSong should return a random index between 0 and playlist's length if shuffled", () => {
    ...
    expect(shuffledIndexesPreviousSong.length).toEqual(
      shuffledIndexesNextSong.length
    );
    let maxCategoryPartition = probMaxIter / songStubs.length;
    maxCategoryPartition += Math.floor(maxCategoryPartition * epsilon);
    for (const nShuffledIndex of shuffledIndexesPreviousSong.concat(
      shuffledIndexesNextSong
    )) {
  ...
  it("playPreviousSong should decrement currentIndex when shuffle is disabled", () => {
    ...
    player.playPreviousSong();
    expect(player.currentIndex).toEqual(
      (((playerIndex - 1) % player.songsInPlayList.length) +
        player.songsInPlayList.length) %
        player.songsInPlayList.length
    );
  });
  ...
  it("shuffleToggle should inverse shuffle state", () => {
    ...
    expect(player.shuffleToggle()).toEqual(!shuffleState);
  });

  it("scrubTime should correctly add delta stepper", () => {
    for (let i = 0; i < MAX_TEST_ITERATIONS; i++) {
      for (let j = MAX_TEST_ITERATIONS; j > 0; j--) {
        player.audio.currentTime = i;
        player.scrubTime(j);
        expect(player.audio.currentTime).toEqual(i + j);
      }
    }
  });
