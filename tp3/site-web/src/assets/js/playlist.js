...
export default class PlayListManager {
  ...
  loadSongs (playlistId) {
    const playlist = this.storageManager.getItemById(
  ...
  playAudio (index) {
    ...
    playButton.classList.remove(
      this.player.audio.paused ? "fa-pause" : "fa-play"
    );
  }
...
window.onload = () => {
  new PlayListManager(new Player(), new StorageManager()).load();
};
